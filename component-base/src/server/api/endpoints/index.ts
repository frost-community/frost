...
import { IDocument, TokenDocument, UserDocument, AppDocument, IPopulatableDocument } from '../../misc/documents';
...
export function registerEndpoint(endpoint: IEndpoint, endpointPath: string, middlewares: RequestHandler[], baseApi: IBaseApi, db: MongoProvider, activeConfigManager: ActiveConfigManager): void {

	// if some scopes is needed, request an AccessToken
		...
		const endpointManager = new EndpointManager(
			...
			try {
				...
				if (endpoint.scopes.length > 0) {
					...
					const appDoc: AppDocument = (req.authInfo as any).app;
					const tokenDoc: TokenDocument = (req.authInfo as any).token;
					const userDoc: UserDocument = (req.user as any);
