...
import { IUserDocumentSoruce, IUserDocument, UserDocument } from '../misc/documents';
...
export default class UserService {
	...
	async create(screenName: string, password: string | null, name: string, description: string, options?: IUserCreateOptions): Promise<UserDocument> {
		...
		const rawDocument: IUserDocument = await this.db.create('base.users', source);
	...
	async findByScreenName(screenName: string): Promise<UserDocument | null> {
		// find (ignore case)
		const rawDocument: IUserDocument | null = await this.db.find('base.users', { screenName: new RegExp(`^${screenName}$`, 'i') });

		return rawDocument ? new UserDocument(rawDocument) : null;
	}

	async findById(userId: string | ObjectId): Promise<UserDocument | null> {
		const rawDocument: IUserDocument | null = await this.db.findById('base.users', userId);
