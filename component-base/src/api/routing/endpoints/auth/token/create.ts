...
import { TokenResponseObject } from 'local/src/api/response/responseObjects';
import { IAppDocument, IUserDocument, AppDocument, TokenDocument } from 'local/src/api/documents';
...
export default define({
	...
	const userDocRaw: IUserDocument = await manager.db.findById('base.users', userId);
	if (!userDocRaw) {
