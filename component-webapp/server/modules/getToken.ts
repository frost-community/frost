...
export default async function(userId: string, appId: string, scopes: string[], activeConfigManager: ActiveConfigManager): Promise<ITokenInfo | null> {
	...
	tokenResult = await axios.post(`${apiBaseUrl}/auth/token/get`, {
		appId: appId,
		userId: userId,
		scopes: scopes
